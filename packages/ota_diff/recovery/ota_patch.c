...
static int backup_map_create(bkup_map_t *bkup_map, size_t ota_size, uint16_t blk_len)
{
    ...
    remain_size     = (size_t)ota_info->patch_partition->len;
...
static int backup_map_add(bkup_map_t *bkup_map, uint16_t blk_idx, uint32_t backup_ed_addr, uint8_t *blk_buf)
{
    ...
    if (fal_partition_erase(ota_info->patch_partition, backup_ee_addr, blk_len) < 0) {
        return -1;
    }

    if (fal_partition_read(ota_info->active_app_partition, backup_ed_addr, blk_buf, blk_len) < 0) {
        return -1;
    }

    if (fal_partition_write(ota_info->patch_partition, backup_ee_addr, blk_buf, blk_len) < 0) {
        return -1;
    }
...
static query_res_t backup_map_query(bkup_map_t *bkup_map, uint16_t blk_idx, uint8_t *blk_buf)
{
    ...
    for (i = 0; i < bkup_map->backup_n; ++i) {
        if (bkup_map->cache[i] == blk_idx) {
            ...
            if (fal_partition_read(ota_info->patch_partition, backup_ee_addr, blk_buf, blk_len) < 0) {
...
int ota_patch(uint32_t patch, size_t patchsize, size_t ota_size)
{
    ...
    if (fal_partition_read(ota_info->patch_partition, patch, (uint8_t *)&patch_hdr, sizeof(patch_hdr_t)) < 0) {
        ERROR();
    }
    ...
    while (blk_cnt--) {
        ...
        if (fal_partition_read(ota_info->patch_partition, patch, (uint8_t *)&patch4blk_hdr, sizeof(patch4blk_hdr_t)) < 0) {
            ERROR();
        }
        ...
        if (fal_partition_read(ota_info->patch_partition, patch, zipped_patch, the_zipped_len) < 0) {
            ERROR();
        }
        ...
        while (N--) {
            ...
            if (X == CMD_COPY || X == CMD_DIFF) { // a COPY or DIFF cmd
                ...
                if (ota_partition_is_pingpong()) {
                    ...
                    if (fal_partition_read(ota_info->backup_app_partition,THE_BUF(0, old_idx, blk_len),
                                            blk_buf, blk_len) < 0) {
                        ERROR();
                    }
                } else {
                    ...
                    if (res == QUERY_RESULT_NOT_EXIST) {
                        if (fal_partition_read(ota_info->active_app_partition,THE_BUF(0, old_idx, blk_len),
                                                blk_buf, blk_len) < 0) {
                            ERROR();
                        }
                    }
        ...
        if (fal_partition_erase(ota_info->active_app_partition, THE_BUF(0, I, blk_len), blk_len) < 0) {
            ERROR();
        }

        if (fal_partition_write(ota_info->active_app_partition, THE_BUF(0, I, blk_len), block, blk_len) < 0) {
            ERROR();
        }
    }
